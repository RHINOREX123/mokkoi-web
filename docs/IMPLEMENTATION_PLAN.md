...
- `[2026-05-02]` React duplicate-key warning fixed (App.tsx:571, 595-611). **Root cause:** `previewRefreshKey` (`useState(0)` at App.tsx:112, bumped by the refresh button) was used as the raw `key` on two sibling components inside the preview wrapper `<div>` at App.tsx:566 — `<PreviewPhoneFrame key={previewRefreshKey}>` at line 571, and `<RuntimeIframePreview {...sharedProps}>` / `<InlineSnackPreview {...sharedProps}>` at line 605/610 where `sharedProps = { key: previewRefreshKey, … }`. Both siblings rendered with `key="0"` on initial mount → "Encountered two children with the same key, `0`" warning. Compounded by the `{...sharedProps}` spread also tripping React's separate "key prop being spread into JSX" warning (same root cause, two diagnostic angles). **Fix:** namespace the keys — `static-${previewRefreshKey}` for `PreviewPhoneFrame`, `live-${previewRefreshKey}` for the IIFE branches; pulled `key` out of `sharedProps` so it's an explicit JSX attribute, never spread. Six-line change, single file. **Provenance:** pre-existing since Week 4 Day 1's IIFE wrapper introduced the `sharedProps` shape (commit `679c1b5`); Day 3 cleanup (`d98b316`) did not introduce or compound it. **Verification:** `npx tsc --noEmit` clean, `npm run build` clean. Live in-browser console verification of the warning's absence deferred — preview browser had no Supabase session and reaching `/app/:projectId` requires auth; reasoning instead from the small mechanical change surface (no logic / protocol / UI change). Will surface naturally next time the canvas is opened. Latent React reconciliation correctness improvement; no user-visible behavior change.
